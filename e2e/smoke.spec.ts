...
test('app boots and the branded Landing renders both actions', async ({ page }) => {
  ...
  await expect(page.getByText(/type .*new/i)).toBeVisible();
  expect(errors, `uncaught page errors: ${errors.join('\n')}`).toHaveLength(0);
});

test('assets are served under the /play/beta/ base', async ({ page }) => {
  const resp = await page.goto('/play/beta/');
  expect(resp?.status()).toBe(200);
  expect(await page.content()).toMatch(/\/play\/beta\/assets\//);
});
