...
# fed2-tools/scripts/muddle for a Docker shim). --profile "" builds only,
# skipping muddlet's desktop-profile deploy (which we don't need here):
#   (cd ../fed2-tools && muddlet --profile "")
...
if [ -n "${LOCAL_PKG:-}" ]; then
  ...
  if [ ! -f "$LOCAL_PKG" ]; then
    ...
    echo "           (cd ../fed2-tools && muddlet --profile \"\")   # needs muddle on PATH; see fed2-tools/scripts/muddle" >&2
    exit 1
  fi
